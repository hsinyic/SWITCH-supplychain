...
def define_components(mod):
    ...
    mod.PRODUCING_GENERATION_PROJECTS = Set(
        dimen=1,
        initialize=lambda m: m.GENERATION_PROJECTS - m.NON_PRODUCING_GENERATION_PROJECTS)
    ##############################

    mod.gen_dbid = Param(mod.GENERATION_PROJECTS, default=lambda m, g: g) #NOT USED
    ...
    mod.gen_energy_source = Param(mod.GENERATION_PROJECTS, 
        validate=lambda m,val,g: val in m.ENERGY_SOURCES or val == "multiple") #NOT USED
    ...
    mod.gen_is_variable = Param(mod.GENERATION_PROJECTS, within=Boolean) # NOT USED except scenario_data 
    mod.gen_is_baseload = Param(mod.GENERATION_PROJECTS, within=Boolean) # used in gen_min_load_fraction, init_gen_availability
    mod.gen_is_cogen = Param(mod.GENERATION_PROJECTS, within=Boolean, default=False) # prevent non-cogen plants from burning pure LSFO after 2017 due to MATS emission restrictions
    mod.gen_is_distributed = Param(mod.GENERATION_PROJECTS, within=Boolean, default=False) #in dispatch.py
    ...
    mod.BASELOAD_GENS = Set(
        initialize=mod.GENERATION_PROJECTS,
        filter=lambda m, g: m.gen_is_baseload[g]) #VARIABLE_GENS and BASELOAD_GENS don't cover the whole space 
    
    mod.CAPACITY_LIMITED_GENS = Set(within=mod.GENERATION_PROJECTS) #NOT USED
    mod.gen_capacity_limit_mw = Param(
        mod.CAPACITY_LIMITED_GENS, within=PositiveReals) #NOT USED
    mod.DISCRETELY_SIZED_GENS = Set(within=mod.GENERATION_PROJECTS)#### NOT USED
    mod.gen_unit_size = Param(
        mod.DISCRETELY_SIZED_GENS, within=PositiveReals) 
    mod.CCS_EQUIPPED_GENS = Set(within=mod.GENERATION_PROJECTS) #carbon capture and storage would eliminate the emission >> see dispatch.py
    ...
    mod.PREDETERMINED_GEN_BLD_YRS = Set(
        dimen=2)
    mod.PREDETERMINED_GEN_BLD_YRS2 = Set(
        dimen=2)
    mod.GEN_BLD_YRS = Set(
        dimen=2,
    ...
    mod.gen_predetermined_cap2= Param(
        mod.PREDETERMINED_GEN_BLD_YRS2,
        within=NonNegativeReals)
    ...
    mod.BuildGen_assign_default_value2 = BuildAction(
        mod.PREDETERMINED_GEN_BLD_YRS2,
        rule=BuildGen_assign_default_value2)
    ...
    mod.GENERATION_PROJECTS_EXTRACTION = Set(dimen=1, initialize=["Wells_Shale_Oil",
        "Wells_Offshore_Oil","Wells_Shale_Gas","Wells_Offshore_Gas",
        "Mines_Coal_Surface"],filter=lambda m, r: r in mod.GENERATION_PROJECTS)
    mod.GENERATION_PROJECTS_EXTRACTION_PERIOD = Set(dimen=2,initialize=mod.GENERATION_PROJECTS_EXTRACTION * mod.PERIODS) 
    # mod.GENERATION_PROJECTS_EXTRACTION_FUEL = Set(dimen=2, within=mod.GENERATION_PROJECTS) 
    mod.GENERATION_PROJECTS_EXTRACTION_ALL = Set(dimen=1, initialize=["Wells_Shale_Oil", "Wells_Conventional_Oil"
        "Wells_Offshore_Oil","Wells_Shale_Gas","Wells_Offshore_Gas","Wells_Conventional_Gas",
        "Mines_Coal_Surface", "Mines_Coal_Underground"],filter=lambda m, r: r in m.GENERATION_PROJECTS)
    ...
    #         if g in m.MULTIFUEL_GENS 
    #         else [m.gen_energy_source[g]]))
    mod.Limited_Fuels = Set(dimen=1, initialize=["Oil", "LNG", "Coal"],filter=lambda m, r: r in m.FUELS)
...
def load_inputs(mod, switch_data, inputs_dir):
    ...
    switch_data.load_aug(
        ...
        param=(mod.gen_dbid, mod.gen_tech, mod.gen_energy_source,
               ...
               mod.gen_is_distributed,mod.gen_hawaii_share))
    ...
    switch_data.load_aug(
        optional=True,
        filename=os.path.join(inputs_dir, 'gen_build_predetermined_Test.tab'),
        auto_select=True,
        index=mod.PREDETERMINED_GEN_BLD_YRS2,
        param=(mod.gen_predetermined_cap2))
    ...
    switch_data.load_aug(
        optional=True,
        filename=os.path.join(inputs_dir, 'non_producing_generation_projects.tab'),
        set=('NON_PRODUCING_GENERATION_PROJECTS'))
    switch_data.load_aug(
        filename=os.path.join(inputs_dir, 'supply_chain_binary.tab'),
        auto_select=True,
        index=mod.GENERATION_PROJECTS,
        param=(mod.extraction_binary, mod.transportation1_binary, 
            mod.transportation2_binary, 
            mod.extraction_binary_offshore, mod.transportation1_binary_offshore, mod.transportation2_binary_offshore,
            mod.extraction_binary_shale,mod.extraction_binary_surface))
    switch_data.load_aug(
        filename=os.path.join(inputs_dir, 'extraction_share.tab'),
        auto_select=True,
        index=mod.GENERATION_PROJECTS_EXTRACTION_PERIOD,
        param=(mod.extraction_share_lb, mod.extraction_share_ub))
    switch_data.load_aug(
        filename=os.path.join(inputs_dir, 'OG_share.tab'),
        auto_select=True,
        index=mod.PERIODS,
        param=(mod.OG_share_lb, mod.OG_share_ub))
    switch_data.load_aug(
        filename=os.path.join(inputs_dir, 'extraction_share_v2.tab'),
        auto_select=True,
        index=mod.FUELS_PERIOD,
        param=(mod.extraction_offshore_share_ub, mod.extraction_shale_share_ub, mod.extraction_surface_ub))
    extraction_fuels_path = os.path.join(inputs_dir, 'extraction_fuel_list.dat')
    if os.path.isfile(extraction_fuels_path):
        switch_data.load(filename=extraction_fuels_path)
    ##############################
...
def post_solve(instance, outdir):
    ...
    write_table(
        instance, instance.PREDETERMINED_GEN_BLD_YRS2, 
        output_file=os.path.join(outdir, "PREDETERMINED_GEN_BLD_YRS2.txt"),
    ...
    write_table(
        instance, instance.PREDETERMINED_GEN_BLD_YRS2, 
        output_file=os.path.join(outdir, "PREDETERMINED_GEN_BLD_YRS2.txt"),
        headings=("GENERATION_PROJECT", "PERIOD", "gen_predetermined_cap2", "gen_predetermined_cap1"),
        values=lambda m, gp: gp + (m.gen_predetermined_cap2[gp],  m.gen_predetermined_cap[gp]))
