...
def define_components(model):
    ...
    #     rule=lambda m, p: m.AnnualEmissions[p] <= m.carbon_cap_tco2_per_yr[p]*5,
    #     doc=("Enforces the carbon cap for generation-related emissions."))

    model.Enforce_cumulative_Carbon_Cap = Constraint(
        rule=lambda m: m.CumulativeEmissions <= 1.16142e+07*0.8,
        # rule=lambda m: m.CumulativeEmissions <= m.BAU_cumulative_carbon_cap*m.cumulative_carbon_cap_scalar,
        doc=("Enforces the carbon cap for generation-related emissions in cumulative sense."))
...
def post_solve(model, outdir):
    ...
    def get_row(model, period):
        row = [period, model.AnnualEmissions[period], model.CumulativeEmissions,
    ...
    reporting.write_table(
        ...
        headings=("PERIOD", "AnnualEmissions_tCO2_per_yr", "cumulative_emission",
